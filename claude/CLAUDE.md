...
## Branch Workflow
- Create the branch as soon as the task scope is decided — before opening any file for editing. Do NOT edit on `main` and migrate the changes to a branch later via stash; that defeats the point of worktrees (main stays dirty, stash slot conflicts across parallel tasks, no isolation). The trigger to branch is the moment a task crosses from exploration/Q&A into "I'm about to edit files that will land in a PR".
